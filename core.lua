...
function MAIRegisterFrameColor(ele)
	if MAIGV("FrameColorEnabled", true) and not MAIGV("nochanges") then
...
local ELESETTINGS = {
	...
	["Interface"] = {
		...
		["MAIBuffFrame"] = {"MAIDebuffFrame", "TemporaryEnchantFrame"},
		["Minimap"] = {"minimapbuttons",},
		--"Post", --"Calendar", --"WorldMapButton"
		["MAIStatusBar"] = {},
		"MawBuffsBelowMinimapFrame", "CastingBarFrame", "ArcheologyDigsiteProgressBar", "MirrorTimer1", "DurabilityFrame", "VehicleSeatIndicator", "MAIMicroButtons", "BagBar", "PlayerPowerBarAlt", "UIWidgetPowerBarContainerFrame", "TalkingHeadFrame", "ChatFrame1", "GameTooltip", "AlertFrame", "BNToastFrame", "GroupLootFrame1", "MAISkills"
	},
	--"EPBar", --"REPBar", --"HonorBar", --"AzeriteBar"
	["InterfaceAdvanced"] = {"UIWidgetTopCenterContainerFrame", "UIWidgetBelowMinimapContainerFrame", "FPSFrame", "PingFrame", "GV", "TorghastPlayerChoiceToggleButton", "MAIMoneyBar", "UIErrorsFrame"}
}
...
function MAISetupMAIMENU()
	...
	for i, v in pairs(ELEMENTS) do
		...
		if MAIGV(v.name .. "Enabled") and _G[v.name] ~= nil then
			mcur = mcur + 1
		end
	end
	...
	local function MAIUpdate()
		if MAIGV("nochanges") then
			...
			if MAIMENU.showilvl:GetChecked() ~= MAIGV("PaperDollFrame" .. "showilvl") then
				MAIMENU.showilvl:SetChecked(MAIGV("PaperDollFrame" .. "showilvl"))
			end
		end
	...
	local function MAIUpdate2()
		if MAIGV("nochanges") then
			...
			if MAIMENU.showitemquality:GetChecked() ~= MAIGV("showitemquality") then
				MAIMENU.showitemquality:SetChecked(MAIGV("showitemquality"))
			end
		end
	...
	function MAITHINKING()
		if MAILoaded and MAITHINKER.tab and (MAITHINKER.toggler ~= MAIGV("toggler") or MAITHINKER.tabsize ~= getn(MAITHINKER.tab) or MAITHINKER.force == true) then
...
function MAIElementSetup(element)
	if MAILoaded and MAIGV(element.name .. "Enabled") then
		if _G[element.name] == nil then
			...
			-- alpha outside of combat
			if MAIGV(element.name .. "alpha") == nil then
				MAISV(element.name .. "alpha", 1)
			end

			-- alpha inside of combat
			if MAIGV(element.name .. "alpha2") == nil then
			...
			function mover.Think()
				if mover.frame == nil and _G[mover.element.name] ~= nil then
					...
					if not InCombatLockdown() or MAIEVENT == "PLAYER_ENTERING_WORLD" then
						...
						-- Setup
						if mover.element.setup ~= nil and (MAIGV("nochanges") and not mover.element.nochanges or not MAIGV("nochanges")) then
				...
				if MAIGV("toggler") then
					...
					if MAIGV(mover.element.name .. "hide") then
						...
						local _, _, _, ppx, ppy = mover:GetPoint()
						ppx = MAIMathR(ppx, 0)
						ppy = MAIMathR(ppy, 0)
						mover.textcenter:SetText("X: " .. ppx .. " Y: " .. ppy)
					end
			...
			if MAIGV("StanceBar" .. "Enabled") and MAIGV("StanceBar" .. "allclasses") then
				...
				function b:MAIThink()
					if MAIIsSpellKnown(spellid1, "MAIThink") and MAIGV("StanceBar" .. "allclasses") and (not b.allclassesplus or (b.allclassesplus and MAIGV("StanceBar" .. "allclassesplus"))) then
						...
						if MAIBUILD ~= "RETAIL" then
							-- soulstone
							if b.soulstone and GetItemCount(6265) then
	...
	if event == "CHAT_MSG_ADDON" then
		...
		if prefix == MAIPREFIX then
			...
			if version == MAIVERSION then
				...
				-- check only once, against spam
				if MAICHECKEDVERSION == false then
...
function MAIUnitXP(unit)
	...
	if MAILoaded and MAIGV("XPTAB") and MAIGV("XPTAB")[target] and MAIGV("XPTAB")[target]["XP"] then return tonumber(MAIGV("XPTAB")[target]["XP"]) end
...
function MAIUnitXPMax(unit)
	...
	if MAILoaded and MAIGV("XPTAB") and MAIGV("XPTAB")[target] and MAIGV("XPTAB")[target]["XPMAX"] then return tonumber(MAIGV("XPTAB")[target]["XPMAX"]) end
...
function MAIFormatValue(val)
	...
	elseif val < 1000000000 then
		return ("%.1fm"):format(val / 1000000)
	elseif val < 1000000000000 then
		return ("%.1fb"):format(val / 1000000000)
	else
		return ("%.1ft"):format(val / 1000000000000)
	end
end
