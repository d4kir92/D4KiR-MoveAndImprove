...
function MAIUpdateActionButton()
	...
	-- MainMenu Handle
	if MAILoaded and (MAIGV("ActionBar" .. 1 .. "Enabled") or MAIGV("ActionBar" .. 5 .. "Enabled") or MAIGV("ActionBar" .. 6 .. "Enabled")) and mmb == false then
		mmb = true
	...
	-- Right ActionButtons
	if MAILoaded and (MAIGV("ActionBar" .. 3 .. "Enabled") or MAIGV("ActionBar" .. 4 .. "Enabled")) and rmb == false then
		rmb = true
	end

	-- Arrows from main bar
	if MAILoaded and MAIGV("ActionBar" .. 2 .. "Enabled") and mbp == false then
		mbp = true
...
function MAICreateBars()
	if IsAddOnLoaded("MoveAny") or IsAddOnLoaded("ImproveAny") then return end
	MAIActionbarsCheckDefault()
...
function GetFlyOutDirection(rows, point)
	...
	-- High |
	if type(rows) == "string" and rows == "max" then
		if point == "TOPRIGHT" or point == "RIGHT" or point == "BOTTOMRIGHT" then return "LEFT" end

		return "RIGHT"
	elseif rows < 6 then
		-- Long _
		if point == "TOPLEFT" or point == "TOP" or point == "TOPRIGHT" then return "DOWN" end
