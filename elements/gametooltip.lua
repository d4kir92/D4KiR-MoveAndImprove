...
function MAISetupGameTooltip()
	...
	-- Fixes: Action[SetPoint] failed because[SetPoint would result in anchor family connection]
	if GameTooltip.OldSetPoint == nil then
	...
	hooksecurefunc(GameTooltip, "SetPoint", function(self, ...)
		...
		else -- HookScript("OnUpdate")
			--and MAIMathR(ofsx, 0) == -97 and MAIMathR(ofsy, 0) == 130 then
			if point == "BOTTOMRIGHT" and (type(uip) == "string" and uip == "UIParent") and relativePoint == "BOTTOMRIGHT" then
